...
function createChatItem(customer) {
    ...
    const statusBadge = getOrderStatusBadge(customer.orderStatus);
    const unreadBadge = customer.totalOrders > 0
        ? `<span class="badge bg-info text-white order-status-badge">
            <i class="bi bi-box-seam"></i> ${customer.totalOrders} đơn
           </span>`
        : '';
    ...
    div.innerHTML = `
        ...
        <div class="chat-item-content">
            ...
            <div class="chat-item-badges mt-1">
                ${statusBadge}
                ${unreadBadge}
            </div>
        </div>
...
function getOrderStatusBadge(status) {
    const badges = {
        'pending': '<span class="badge bg-warning text-dark order-status-badge"><i class="bi bi-clock"></i> Chờ xử lý</span>',
        'shipping': '<span class="badge bg-primary order-status-badge"><i class="bi bi-truck"></i> Đang giao</span>',
        'completed': '<span class="badge bg-success order-status-badge"><i class="bi bi-check-circle"></i> Hoàn thành</span>',
    };
